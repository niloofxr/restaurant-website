...
function renderMenu(){
  if(!$('#menuGrid')) return;
  if(!$('#categoryStyles')){const style=document.createElement('style');style.id='categoryStyles';style.textContent='.menu-category{margin:0 0 70px}.category-heading{display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid var(--line);padding-bottom:14px;margin-bottom:0}.category-heading>span{font:10px "DM Mono";color:var(--muted);text-transform:uppercase}.category-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:0 60px}.drinks-page .category-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:0 40px}.drinks-page .dish-actions{display:flex}.drinks-page .dish-actions>div{margin-top:0}@media(max-width:900px){.drinks-page .category-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:720px){.category-grid,.drinks-page .category-grid{grid-template-columns:1fr}}';document.head.appendChild(style)}
  const categories=document.body.dataset.menuType==='drinks'?['Drinks']:['Appetizers','Main items','Desserts','Daily specials'];
  if(document.body.dataset.menuType==='drinks') $('#menuGrid').style.display='block';
  const dishCard=d=>{const rating=ratings[d.id]||0;return `<article class="dish"><div class="dish-top"><div><h3>${d.name}</h3><p>${d.description}</p></div><span class="dish-price">${money(d.price)}</span></div><div class="dish-actions"><span class="dish-tag">${d.tag}</span><div><div class="stars" aria-label="Rate ${d.name}">${[1,2,3,4,5].map(n=>`<button class="${n<=rating?'active':''}" data-dish="${d.id}" data-rate="${n}">★</button>`).join('')}</div><button class="add" type="button" data-add="${d.id}" onclick="addToTray('${d.id}')">+ Add to tray</button></div></div></article>`};
  const gridStyle=document.body.dataset.menuType==='drinks'?' style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0 60px"':'';
  $('#menuGrid').innerHTML=categories.map(category=>`<section class="menu-category"><div class="category-heading"><div class="section-kicker">${category}</div><span>${MENU.filter(d=>d.category===category).length} transmissions</span></div><div class="category-grid"${gridStyle}>${MENU.filter(d=>d.category===category).map(dishCard).join('')}</div></section>`).join('');
...
if(siteNav) siteNav.innerHTML='<a href="index.html">Home</a><a href="menu.html">Menu</a><a href="drinks.html">Drinks</a><a href="reservation.html">Book a Table</a>';
