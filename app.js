...
function renderMenu(){
  ...
  $('#menuGrid').innerHTML=categories.map(category=>`<section class="menu-category"><div class="category-heading"><div class="section-kicker">${category}</div><span>${MENU.filter(d=>d.category===category).length} transmissions</span></div><div class="category-grid">${MENU.filter(d=>d.category===category).map(dishCard).join('')}</div></section>`).join('');
